...
we can assume that sun rays are parallel. After crossing the lens all converge in the same *burning* spot. 
...
Let's draw a horizontal line from left to right. This line will be called *axis*. 
...
+ This intersection point *does not* depend on the initial distance $$d_0$$.
+ We call this point the *focal point*.
+ It is located on the axis, at a distance $$f$$ that is called *focal distance*
...
+ rays represented by *vectors*: ordered list of values. Written vertically. Fixed size.
...
+ each transformation of every component of the ray has the same shape: $$t(i+1)=\alpha d(i) + \beta t(i)$$. This is a way to combine two vectors that will be very useful later. It is called *dot product*.
