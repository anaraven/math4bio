...
Something easy and useful is to "turn the matrix sideways". This is called *transposition*. If $𝐀∈ ℝ^{m× n},$ then we can build a new matrix $𝐀ᵀ∈ ℝ^{n× m}$ such that each component of $𝐀ᵀ$ is defined by
$$(Aᵀ)_{ij} = a_{ji}$$
